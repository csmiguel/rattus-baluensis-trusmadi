...
int maketarget(char *input_len,char *input_sst,char *output){
	int cloc,cstat,cpop,cpop2,cdna,i,	//iterators
	...
	inp_len = fopen(input_len,"r");						//input filename
	if(inp_len == NULL)
		return 1; //cannot open .len file
	inp_sst = fopen(input_sst,"r");						//input .sst
	if(inp_sst == NULL)
		return 2; //cannot open .sst file
	...
	for(cloc=0;cloc<nloc;++cloc){
		...
		if(ltype[cloc] =='M' || ltype[cloc]=='m'){
			...
			if(data->lsites[cloc]!=0)
				...
				return 8; //sequence data without info, this should be discarded
		}
	}
